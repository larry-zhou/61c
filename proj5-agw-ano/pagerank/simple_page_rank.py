...
class SimplePageRank(object):
    ...
    def compute_pagerank(self, num_iters):
        ...
        for i in range(0, num_iters):
            nodes = self.update_weights(nodes, num_nodes) # this doesn't match what i did
    ...
    def update_weights(nodes, num_nodes): 
        ...
        def distribute_weights((node, (weight, targets))):
            ...
            temp = []
            temp.append((node, (targets, 0.05 * weight)))
            if len(targets) == 0:
                for n in range(num_nodes):
                    if n != node:
                        temp.append((n, ([], 0.85 * weight/(num_nodes-1))))
                return temp
            for targ in targets:
                temp.append((targ, ([], 0.85 * weight/len(targets))))
            return temp
