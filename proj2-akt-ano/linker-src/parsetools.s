...
hex_to_str:
	# YOUR CODE HERE
	addi $t0, $a1, 9 #add 9 to the buffer were writing in to
	sb $0, 0($t0) #save 0 into the first spot
	li $t1, 10 # load 10 into t1
	addiu $t0, $t0, -1 #move t0 over by 1 spot
	sb $t1, 0($t0) #save $t1 into this new spot
	addiu $t0, $t0, -1 	#move t0 over by 1 spot
bit_loop:
	andi $t1, $a0, 0xF #becaue f is 1111, we can andi with $a0 to get the last 4 bits
	blt $t1, 10, end_hex #check if the chunk is less than 10
	addi $t1, $t1, 0x27 #if its not, add 39 to it
end_hex:
	addi $t1, $t1, 0x30
	sb $t1, 0($t0)
	srl $a0, $a0, 4 #shift over the next set of bits
	addi $t3, $a1, -1
	addi $t0, $t0, -1
	bne $t0, $t3, bit_loop # if they're the same, don't loop
