...
inst_needs_relocation:
        # YOUR CODE HERE
        li $v0, 0   
        srl $t3, $a0, 26 #shift mips instruction 26 times to right to get opcode
        beq $t3, 2, needs_relocation #check opcode against 2 or 3 (jump), (jump and link)
        beq $t3, 3, needs_relocation #still checking opcode    
        jr $ra
...
relocate_inst:
        ...
        addi $a0, $a3, 0
        jal symbol_for_addr             
        beq $v0, 0, ret_error #if symbol for addr returns 0, nothing was found
        move $a1, $v0
        move $a0, $a2
        jal addr_for_symbol                     
        beq $v0, -1, ret_error #if addr for symbol returns -1, there was an error
        andi $v0, $v0, 0x0FFFFFFF #and the result with 0x0FFFFFF to get the bottom 28 bits
        srl $v0, $v0, 2 #shift it over twice and make it 26 bits long
        lw $a0, 4($sp)
        andi $a0, $a0, 0xFC000000 #now and $s0 with FC to get the top 6 bits 
        or $v0, $v0, $a0 #or the results to get 32 bits
        j relct_return
ret_error:
        li $v0, -1
relct_return:   
