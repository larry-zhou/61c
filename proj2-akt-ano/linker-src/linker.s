...
write_machine_code:
	...
	sw $s6, 28($sp) #instruction
	sw $s5, 24($sp) #byte offset
	sw $s0, 20($sp)
	sw $s1, 16($sp)
	sw $s2, 12($sp)
	sw $s3, 8($sp)
...
write_machine_code_next_inst:
	...
	move $a0, $v1
	li $a1, 16
	jal parse_int #takes in string containing number ($a0) and base 10 or 16($a1)
	move $s6, $v0
	...
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $s6	#instruction
	move $a1, $s5	#byte offset
	move $a2, $s2	#symbol table
	move $a3, $s3	#relocation table
	jal relocate_inst	#call relocate_inst
	beq $v0, -1, write_machine_code_error	#if relocate_inst returns 1 something went wrong
	move $s6, $v0 	#store relocated instruction into s6

write_machine_code_to_file:
	# 6. Write the instruction into a string buffer via hex_to_str():
	# YOUR_INSTRUCTIONS_HERE 
	move $a0, $s6	#move instruction into a0 again
	la $a1, hex_buffer	#the buffer we will write
	jal hex_to_str	#convert hex ($a0) and store it into hex_buffer ($a1)
