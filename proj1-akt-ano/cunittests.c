...
void simple_checkout_test(void)
{
    ...
    retval = beargit_checkout("something", 1);
    CU_ASSERT(0==retval);

    retval = beargit_checkout("something2", 1);
    CU_ASSERT(0==retval);

    retval = beargit_checkout("something2", 1);
    CU_ASSERT(1==retval);

    retval = beargit_checkout("something", 0);
    CU_ASSERT(0==retval);

    retval = beargit_checkout("master", 0);
    CU_ASSERT(0==retval);

    retval = beargit_checkout("not_something", 0);
    CU_ASSERT(1==retval);
...
void simple_merge_test(void)
{
    ...
    FILE* asdfg = fopen("asdfg.txt", "w");
    fclose(asdfg);

    beargit_add("asdfg.txt");
    ...
    retval = beargit_merge("something2");
    CU_ASSERT(0==retval);

    free_commit_list(&commit_list);
}

void merge_output_test(void)
{
    struct commit* commit_list = NULL;
    int retval;
    retval = beargit_init();
    ...
    FILE* fstdout = fopen("TEST_STDOUT", "r"); //check if there is conflicting files.
    CU_ASSERT_PTR_NOT_NULL(fstdout);
...
int cunittester()
{
   ...
   /* Add tests to the Suite #2 */
   if (NULL == CU_add_test(pSuite3, "Checkout branch test", simple_checkout_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
   ...
   /* Add tests to the Suite #2 */
   if (NULL == CU_add_test(pSuite4, "Merge branch test", simple_merge_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
   ...
   /* Add tests to the Suite #2 */
   if (NULL == CU_add_test(pSuite5, "Merge output test", merge_output_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
